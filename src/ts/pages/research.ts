...
import { loadAllReports, loadReport, type Report } from '../data-loader';

const FILTERS = ['All', 'History', 'Scholarship', 'Gaps', 'AI', 'Archives', 'Diaspora', 'Culture', 'Contemporary', 'Language', 'Personas', 'Innovation', 'Competitive'];
  ...
  app.innerHTML = `
    <div class="tool-page container">
      ...
      <div style="display: flex; gap: 12px; margin-bottom: 24px; flex-wrap: wrap;">
        ...
        <a href="#/tools/ask" class="btn btn-ghost" style="font-size: 0.8125rem; padding: 8px 16px;">${t('ask.title')}</a>
      </div>
...
export function renderResearchDetail(): void {
  ...
  app.innerHTML = `<div class="container"><p style="padding: 64px 0; color: var(--text-tertiary);">${t('common.loading')}</p></div>`;

  loadReport(id).then(report => {
    if (!report) { navigate('/research'); return; }
    renderDetail(report, lang);
  });
}

function renderDetail(report: Report, lang: 'en' | 'jp'): void {
  const app = document.getElementById('app')!;
  {
