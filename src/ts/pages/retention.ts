...
function getRiskLevel(percentage: number): { level: string; label: { en: string; jp: string } } {
  if (percentage >= 65) return { level: 'low', label: { en: 'Low Risk', jp: '低リスク' } };
  if (percentage >= 35) return { level: 'medium', label: { en: 'Medium Risk', jp: '中リスク' } };
  return { level: 'high', label: { en: 'High Risk', jp: '高リスク' } };
}
...
function getRecommendations(lang: string): string[] {
  ...
  if (values.discipleship < 5) {
    recs.push(lang === 'en'
      ...
      : '弟子訓練の深さが著しく不足しています。出発前に体系的な弟子訓練プログラムを修了することを優先してください。');
  }
  if (values.preparation < 4) {
    recs.push(lang === 'en'
      ...
      : '帰国前の準備が不十分です。帰国準備ツールを使って個別のリターンキットを作成してください。');
  }
  if (values.connections < 4) {
    recs.push(lang === 'en'
      ? 'Japan-side connections are weak. Connect this student to JCFN (Japanese Christian Fellowship Network) and at least 2-3 trusted Christian contacts in their destination city.'
      : '日本側のつながりが弱いです。JCFN（日本クリスチャン・フェローシップ・ネットワーク）と、帰国先の信頼できる2〜3人のクリスチャンに紹介してください。');
  }
  if (values.relational < 5) {
    recs.push(lang === 'en'
      ...
      : '関係的サポートが基準以下です。海外のメンターと定期的な連絡スケジュールを確立してください。');
  }
...
function showResult(): void {
  ...
  resultArea.innerHTML = `
    <div class="result-card">
      ...
      <p class="risk-level ${risk.level}">${risk.label[lang as 'en' | 'jp']}</p>
    </div>

    <div style="margin-top: 24px;">
      <h3 style="font-size: 1rem; margin-bottom: 16px;">${lang === 'en' ? 'Recommendations' : 'おすすめ'}</h3>
      ${recs.map(r => `
